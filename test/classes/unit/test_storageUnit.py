...
def test_init_storage_unit_base(problem, battery_info):
    # Tests the creation of a storage unit. It's a battery
    test_storage_unit = StorageUnit('Battery', battery_info, problem)
...
def test_init_storage_unit_advanced(problem, lhs_info):
    # Tests the creation of a more advanced storage unit, based on hydrogen storage
    test_storage_unit = StorageUnit('LHS', lhs_info, problem)
    assert isinstance(test_storage_unit, StorageUnit)
...
def test_create_info_charging_unit(problem, lhs_info):
...
def test_create_info_discharging_unit(problem, lhs_info):
    # Tests the creation of the charging unit of the hydrogen storage
    test_storage_unit = StorageUnit('LHS', lhs_info, problem)
...
def test_create_auxiliary_units(problem, battery_info, lhs_info):
    # Tests the creation of both auxiliary units
    test_storage_unit = StorageUnit('LHS', lhs_info, problem)
    test_charging_unit_info = test_storage_unit.create_auxiliary_unit_info('LHS', lhs_info, 'Charging')
    test_discharging_unit_info = test_storage_unit.create_auxiliary_unit_info('LHS', lhs_info, 'Discharging')
    ...
    temp_power = lhs_info['Max energy']*100.0
    assert test_charging_unit.max_installed_power == {'Hydrogen': -temp_power, 'LiquidHydrogen': temp_power, 'Electricity': -temp_power*(1-lhs_info['Charging unit info']['Efficiency'])}
    assert test_charging_unit.name == lhs_info['Charging unit info']['Name']
    ...
    assert test_discharging_unit.name ==lhs_info['Discharging unit info']['Name']
...
@fixture
def battery_info():
    return {'Type': 'StorageUnit', 'Specific CAPEX': 140, 'Lifetime': 25, 'Layers': ['Electricity'], 'Main layer': 'Electricity',
...
@fixture
def lhs_info():
    return {'Type': 'StorageUnit', 'Specific CAPEX': 40, 'Lifetime': 25, 'Layers': ['Hydrogen'], 'Main layer': 'Hydrogen',
