...
def test_problem_1(tmp_path):
    ...
    """
    problem_number = 1
    problem_folder = os.path.join(tmp_path, f'test_problem_{problem_number}')
...
def test_problem_2(tmp_path):
    ...
    problem.run()
    assert problem.ampl_problem.solve_result == "solved"
    assert math.isclose(problem.ampl_problem.get_variable('OPEX').value(), -752, abs_tol = 10)
