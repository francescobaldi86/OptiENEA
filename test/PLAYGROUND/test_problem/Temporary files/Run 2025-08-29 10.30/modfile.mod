...
subject to componentSizing{u in standardUtilities, l in  mainLayerOfUnit[u],
...
subject to market_limits{u in markets, l in  layersOfUnit[u], t in
  timeSteps} : POWER_MAX[u,l] <= power[u,l,t] <= 0;
...
subject to storage_cyclic_constraint{u in storageUnits, l in  layersOfUnit[u]
  } : energyStorageLevel0[u,l] >=  if STORAGE_CYCLIC_ACTIVE == 1 then
