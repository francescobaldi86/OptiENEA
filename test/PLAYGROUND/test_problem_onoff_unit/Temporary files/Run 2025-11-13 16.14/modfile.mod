...
set unitsOnOff within nonmarketUtilities;
...
param ENERGY_PRICE_VARIATION{m in markets, l in  layersOfUnit[m], t in
  timeSteps}  default 1;
param BIG_M default 100;
param POWER_MIN{u in nonmarketUtilities}  default 0;
...
var CAPEX;
var TOTEX;
var ips_t{u in unitsOnOff, t in timeSteps}  binary;
minimize obj: TOTEX;
...
subject to minimum_installed_power{u in nonmarketUtilities} : size[u] >=
  POWER_MIN[u];
...
subject to selling_market_limits{u in markets, l in  layersOfUnit[u], t in
  ...
  power[u,l,t] <= 0;
subject to component_load_onoff{u in unitsOnOff, l in  mainLayerOfUnit[u],
  t in timeSteps} : size[u] + (ips_t[u,t] - 1)*BIG_M*abs(POWER_MAX[u,l])
   <= ics[u,t]*abs(POWER_MAX[u,l]);
subject to component_load_onoff_2{u in unitsOnOff, t in timeSteps} : ics[u,t]
   <= ips_t[u,t];
