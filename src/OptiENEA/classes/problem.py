...
class Problem:
      ...
      def load_units_data_from_file(self, filename = ''):
            # Loads the tile containing data about problem units and stores the information in a dictionary
            if filename == '':
                  filename = f'{self.project_folder}\\data\\units.txt'
            return read_input_from_file({}, filename)

      def parse_units_dict(self, dict_units: dict):
            # Reads the data previously temporarily stored in a dictionary about the different units and stores them in the main problem structure
            for unit in dict_units:
                  self.add_unit(unit)
                  
      def add_unit(self, unit: dict):
            # Adds a unit to the units list
            self.units.append(Unit(unit_data = unit))
